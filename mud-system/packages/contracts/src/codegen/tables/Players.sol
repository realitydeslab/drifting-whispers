...
library Players {
   ...
   * @notice Get status.
   */
  function getStatus(bytes32 key) internal view returns (uint256 status) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Get status.
   */
  function _getStatus(bytes32 key) internal view returns (uint256 status) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Set status.
   */
  function setStatus(bytes32 key, uint256 status) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Set status.
   */
  function _setStatus(bytes32 key, uint256 status) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function set(bytes32 key, uint256 x, uint256 y, uint256 status) internal {
    bytes memory _staticData = encodeStatic(x, y, status);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 key, uint256 x, uint256 y, uint256 status) internal {
    bytes memory _staticData = encodeStatic(x, y, status);
  ...
  function set(bytes32 key, PlayersData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.status);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 x, uint256 y, uint256 status) {
    x = (uint256(Bytes.slice32(_blob, 0)));

    y = (uint256(Bytes.slice32(_blob, 32)));

    status = (uint256(Bytes.slice32(_blob, 64)));
  }
  ...
  function decode(
    ...
    (_table.x, _table.y, _table.status) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(uint256 x, uint256 y, uint256 status) internal pure returns (bytes memory) {
    return abi.encodePacked(x, y, status);
  }
